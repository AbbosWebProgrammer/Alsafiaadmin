...
function MainPagePromoForHomePageSlider() {
    ...
    useEffect(() => {
        axios.get(API+"api/MainPagePromoForHomePageSlider/"+`${window.location.pathname.split('/')[3]}`).then((res) => {
            setcarusel(res.data)
    ...
    function hompagecarusel(){
        ...
        axios.put(API+'api/MainPagePromoForHomePageSliderJson/' +`${window.location.pathname.split('/')[3]}`+'/',{"data":editcarousel}).then(res => {
            window.location.replace("/admin/getMainPagePromoForHomePageSlider/");
        })
...
export default MainPagePromoForHomePageSlider;
