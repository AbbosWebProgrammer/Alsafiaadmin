...
function ShoppingDayForHomePageCarousel() {
    ...
    useEffect(() => {
        axios.get(API+"api/ShoppingDayForHomePageCarousel/"+`${window.location.pathname.split('/')[3]}`).then((res) => {
            setcarusel(res.data)
    ...
    const inputBrandChanged=(e) =>{
        ...
        editcarousel.brand=k
        console.log(editcarousel.brand)
    }

    function hompagecarusel(){
        console.log(editcarousel)
        axios.put(API+'api/ShoppingDayForHomePageCarouselJson/' +`${window.location.pathname.split('/')[3]}`+'/',{"data":editcarousel}).then(res => {
            window.location.replace("/admin/getShoppingDayForHomePageCarousel");
        })
    }
        ...
        <div className="ShoppingDayForHomePageCarousel">
            <div className="container">
                <div className="row">
                    ...
                    <div className="col">
                        {editcarousel.image!==''?
                            <img style={{height:"135px"}} src={editcarousel.image} alt=""/>
                            :
                            <img style={{height:"135px"}} src={carusel.image} alt=""/>
                            }
                    </div>
                        ...
                        <select name="brand" id="brand" onChange={inputBrandChanged} className="form-select" multiple={true}>
                            {brand.map(item => {
                               return ( carusel.brand?carusel.brand.includes(item.id)?
