...
function GetHomepPageCarusel() {
    ...
    useEffect(() => {
        axios.get(API+"api/MainPagePromoForHomePage/").then((res) => {
                setCarusel(res.data)
    ...
    function deleted() {
        carousel.forEach(item => {
            if (item.id === deleteItem.id) {
                axios.delete(API+`api/MainPagePromoForHomePage/${deleteItem.id}`)
                    .then(res => {
                        console.log(res)
                    })
                setInputV('')
                    ...
                    <table class="table table-striped align-middle table-bordered ">
                        ...
                        {carousel.length>0?carousel.map(item => {
                            return <tr>
                                ...
                                <td><Link className="btn btn-warning" to={"/admin/editMainPagePromoForHomePage/"+`${item.id}`}>Edit</Link></td>
                ...
                <div className="modal-dialog">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h4 className="text-center">Delete Home Page Promo</h4>
                            {/*<button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"/>*/}
                        </div>
                        <div className="modal-body">
                            <h6>
                            Home Page Promo va uning qo'shimchalari o'chgandan so'ng qaytarib bo'lmaydi.
                                <br/>
